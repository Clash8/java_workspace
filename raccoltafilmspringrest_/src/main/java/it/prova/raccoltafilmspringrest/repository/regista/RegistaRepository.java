...
public interface RegistaRepository
	...
	@Query(value = "SELECT r.* " + "FROM regista r " + "WHERE ((:nome IS NULL OR LOWER(r.nome) LIKE CONCAT('%', :nome, '%'))  "
			+ "AND (:cognome IS NULL OR LOWER(r.cognome) LIKE CONCAT('%', :cognome, '%'))) "
			+ "AND (:datadinascita IS NULL OR r.datadinascita > :datadinascita) "
			+ "AND (:sesso IS NULL OR r.sesso = :sesso) "
			+ "AND (:nickname IS NULL OR LOWER(r.nickname) LIKE CONCAT('%', :nickname, '%'))", countQuery = "SELECT r.* "
					+ "FROM regista r " + "WHERE ((:nome IS NULL OR LOWER(r.nome) LIKE CONCAT('%', :nome, '%'))  "
					+ "AND (:cognome IS NULL OR LOWER(r.cognome) LIKE CONCAT('%', :cognome, '%'))) "
					+ "AND (:datadinascita IS NULL OR r.datadinascita > :datadinascita) "
					+ "AND (:sesso IS NULL OR r.sesso = :sesso) "
					+ "AND (:nickname IS NULL OR LOWER(r.nickname) LIKE  CONCAT('%', :nickname, '%'))", nativeQuery = true)
